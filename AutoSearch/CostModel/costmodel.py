...
from CostModel import NetworkSize as nws
from torch.utils.data import Dataset, DataLoader
from CostModel.sample_load import load_from_dir
from CostModel.Weights import Weights
...
class Net(nn.Module):
	...
	def load_weights(self, weights):
		...
		# Each buffer
		for i, name in enumerate(weights.network):
			data = torch.tensor(weights.weights[name + '_data'])

			dimension = weights.weights[name + '_dimension']
			permute_list = [dim for dim in range(dimension[0])]
			permute_list.reverse()

			extent = list(weights.weights[name + '_extent'])
			extent.reverse()
			data = data.reshape(extent)

			data = data.permute(permute_list)
			data = data.reshape(params[i].shape)
			# params[i] = nn.Parameter(data)
			# params[i] = nn.Parameter(torch.zeros(params[i].shape))
			params[i].data.copy_(data)


def train_cost_model(data_path, weight_path, learning_rate=0.001, train_iters=200):
	# 加载数据
	# data = CSVDataset("../samples/batch_35")
	data = CSVDataset(data_path)
	...
	try:
		weights.load_from_file(weight_path)  
		net = weights.load_in_model(net)
	except:
		print('initialize random weights')

	# 定义优化器
	optimizer = optim.Adam(net.parameters(), lr=learning_rate, betas=(0.9, 0.999), eps=1e-5)
	...
	weights.save_to_json('./{}.json'.format(weight_path))
	weights.save_to_file(weight_path)
