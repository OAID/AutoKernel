...
/*
    if(prerun_graph_multithread(graph, atoi(std::getenv("NumClusterLite")), atoi(std::getenv("NumThreadLite"))) < 0)
    {
...
// #ifdef CONFIG_LITE_TEST
    std::cout << "Number Thread  : [" << num_thread << "]\n" << std::endl;
    std::cout << "Number Cluster : [" << TENGINE_CLUSTER_LITTLE << "]\n" << std::endl;
